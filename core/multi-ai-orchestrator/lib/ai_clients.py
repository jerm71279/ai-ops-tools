...
class FaraCLI(BaseAIClient):
    ...
    def execute(
        ...
        **kwargs
        ...
        fara_cli_path = self.fara_dir / ".venv" / "bin" / "fara-cli"
        cmd = [
            str(fara_cli_path),
            "--task", task,
            "--max_rounds", str(max_steps)
        ]

        if url:
            cmd.extend(["--start_page", url])

        if screenshot_dir:
            cmd.extend(["--downloads_folder", screenshot_dir])
            cmd.append("--save_screenshots")
